...
func (g *Game) useScrewdriver() {
	...
	for i, dalek := range g.daleks {
		...
		if dx <= 1 && dy <= 1 && (dx != 0 || dy != 0) {
			...
			g.screwdriverTargets = append(g.screwdriverTargets, dalek)
		}
	}
...
func (g *Game) checkCollisions() {
	...
	finalDaleks := make([]Position, 0, len(newDaleks))
	...
	for i, dalek := range newDaleks {
		...
		for j, other := range newDaleks {
			if i != j && dalek.X == other.X && dalek.Y == other.Y {
				collided = true
				g.score += 2
				collidedPositions[dalek] = true
				break
...
func (g *Game) Update() error {
	...
	if g.screwdriverAnimation {
		g.screwdriverTimer += 1.0 / 60.0 // Assuming 60 FPS
