...
func (g *Game) lastStand() {
	...
	for iteration := 0; iteration < maxIterations && len(g.daleks) > 0; iteration++ {
		...
		// Wait for movement animation to complete before next iteration
		for g.daleksMoving {
			// This would need to be handled differently in the actual game loop
			// For now, we'll just continue with the logic
		}
		...
		for _, dalek := range g.daleks {
			...
			nextPos := Position{X: dalek.GridPos.X + dx, Y: dalek.GridPos.Y + dy}
...
func (g *Game) Update() error {
	...
	if g.screwdriverAnimation {
		...
		if g.screwdriverTimer >= 0.8 { // Animation duration: 0.8 seconds
	...
	case StatePlaying:
		// Only allow input if daleks are not moving
		if !g.daleksMoving {
			// Movement keys
...
func (g *Game) drawHUD(screen *ebiten.Image) {
	// Status information with black text
